...
def generator_unet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        e77 = SE_ResNeXt(e7)
        d11 = SE_ResNeXt(d2)
        d1 = tf.concat([instance_norm(d11, 'g_bn_d1'), e77], 3)
        ...
        e66 = SE_ResNeXt(e6)
        d22 = SE_ResNeXt(d2)
        d2 = tf.concat([instance_norm(d2, 'g_bn_d2'), e66], 3)
        ...
        e55 = SE_ResNeXt(e5)
        d33 = SE_ResNeXt(d3)
        d3 = tf.concat([instance_norm(d33, 'g_bn_d3'), e55], 3)
        # d3 is (8 x 8 x self.gf_dim*8*2)

        d4 = deconv2d(tf.nn.relu(d3), options.gf_dim*8, name='g_d4')
        e44 = SE_ResNeXt(e4)
        d44 = SE_ResNeXt(d4)
        d4 = tf.concat([instance_norm(d44, 'g_bn_d4'), e44], 3)
        # d4 is (16 x 16 x self.gf_dim*8*2)

        d5 = deconv2d(tf.nn.relu(d4), options.gf_dim*4, name='g_d5')
        e33 = SE_ResNeXt(e3)
        d55 = SE_ResNeXt(d5)
        d5 = tf.concat([instance_norm(d55, 'g_bn_d5'), e33], 3)
        # d5 is (32 x 32 x self.gf_dim*4*2)

        d6 = deconv2d(tf.nn.relu(d5), options.gf_dim*2, name='g_d6')
        e22 = SE_ResNeXt(e2)
        d66 = SE_ResNeXt(d6)
        d6 = tf.concat([instance_norm(d66, 'g_bn_d6'), e22], 3)
        # d6 is (64 x 64 x self.gf_dim*2*2)

        d7 = deconv2d(tf.nn.relu(d6), options.gf_dim, name='g_d7')
        e11 = SE_ResNeXt(e1)
        d77 = SE_ResNeXt(d7)
        d77 = tf.concat([instance_norm(d77, 'g_bn_d7'), e11], 3)
        # d7 is (128 x 128 x self.gf_dim*1*2)

        d8 = deconv2d(tf.nn.relu(d77), options.output_c_dim, name='g_d8')
